...
from syncord.api.http import HTTPClient, Routes, to_bytes
from syncord.types.channel import Channel
from syncord.types.guild import (AuditLogEntry, Guild, GuildBan, GuildEmbed,
                                 GuildEmoji, GuildMember, Integration,
                                 PruneCount, Role)
from syncord.types.invite import Invite
from syncord.types.message import Message
from syncord.types.oauth import Application, Connection
from syncord.types.user import User
from syncord.types.voice import VoiceRegion
from syncord.types.webhook import Webhook
from syncord.util.functional import optional
...
class APIClient(LoggingClass):
    ...
    def channels_messages_list(
        self, channel, around=None, before=None, after=None, limit=50
    ):
        r = self.http(
            Routes.CHANNELS_MESSAGES_LIST,
            dict(channel=channel),
            params=optional(
                around=around,
                before=before,
    ...
    def channels_messages_get(self, channel, message):
        r = self.http(
            Routes.CHANNELS_MESSAGES_GET, dict(channel=channel, message=message)
        )
    ...
    def channels_messages_create(
        ...
        if attachments:
            ...
            r = self.http(
                Routes.CHANNELS_MESSAGES_CREATE, dict(channel=channel), json=payload
            )

        return Message.create(self.client, r.json())

    def channels_messages_modify(
        self, channel, message, content=None, embed=None, flags=None, sanitize=False
    ):
    ...
    def channels_messages_delete(self, channel, message):
        self.http(
            Routes.CHANNELS_MESSAGES_DELETE, dict(channel=channel, message=message)
        )

    def channels_messages_delete_bulk(self, channel, messages):
        self.http(
            Routes.CHANNELS_MESSAGES_DELETE_BULK,
            dict(channel=channel),
            json={"messages": messages},
        )

    def channels_messages_reactions_get(
        self, channel, message, emoji, after=None, limit=100
    ):
        r = self.http(
    ...
    def channels_messages_reactions_create(self, channel, message, emoji):
        self.http(
            Routes.CHANNELS_MESSAGES_REACTIONS_CREATE,
            dict(channel=channel, message=message, emoji=emoji),
        )
    ...
    def channels_messages_reactions_delete_all(self, channel, message):
        self.http(
            Routes.CHANNELS_MESSAGES_REACTIONS_DELETE_ALL,
            dict(channel=channel, message=message),
        )

    def channels_permissions_modify(
        self, channel, permission, allow, deny, typ, reason=None
    ):
        self.http(
            Routes.CHANNELS_PERMISSIONS_MODIFY,
            dict(channel=channel, permission=permission),
            json={
                "allow": allow,
                "deny": deny,
                "type": typ,
    ...
    def channels_invites_create(
        ...
        r = self.http(
            Routes.CHANNELS_INVITES_CREATE,
            dict(channel=channel),
            json={
                "max_age": max_age,
                "max_uses": max_uses,
    ...
    def channels_webhooks_create(self, channel, name=None, avatar=None):
        r = self.http(
            Routes.CHANNELS_WEBHOOKS_CREATE,
            dict(channel=channel),
            json=optional(
                name=name,
                avatar=avatar,
    ...
    def guilds_members_add(
        self, guild, member, access_token, nick=None, roles=None, mute=None, deaf=None
    ):
        ...
        self.http(
            Routes.GUILDS_MEMBERS_ADD, dict(guild=guild, member=member), json=payload
        )

    def guilds_members_kick(self, guild, member, reason=None):
        self.http(
            Routes.GUILDS_MEMBERS_KICK,
            dict(guild=guild, member=member),
            headers=_reason_header(reason),
        )
    ...
    def guilds_roles_modify_batch(self, guild, roles, reason=None):
        r = self.http(
            Routes.GUILDS_ROLES_MODIFY_BATCH,
            dict(guild=guild),
            json=roles,
            headers=_reason_header(reason),
        )
    ...
    def guilds_integrations_create(self, guild, type, id):
        r = self.http(
            Routes.GUILDS_INTEGRATIONS_CREATE,
            dict(guild=guild),
    ...
    def guilds_integrations_delete(self, guild, integration):
        self.http(
            Routes.GUILDS_INTEGRATIONS_DELETE,
            dict(guild=guild, integration=integration),
        )

    def guilds_integrations_sync(self, guild, integration):
        self.http(
            Routes.GUILDS_INTEGRATIONS_SYNC, dict(guild=guild, integration=integration)
        )
    ...
    def guilds_auditlogs_list(
        self, guild, before=None, user_id=None, action_type=None, limit=50
    ):
        r = self.http(
        ...
        return AuditLogEntry.create_map(
            self.client, r.json()["audit_log_entries"], users, webhooks, guild_id=guild
        )
    ...
    def invites_delete(self, invite, reason=None):
        r = self.http(
            Routes.INVITES_DELETE, dict(invite=invite), headers=_reason_header(reason)
        )
        return Invite.create(self.client, r.json())
