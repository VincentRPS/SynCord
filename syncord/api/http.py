...
from syncord import VERSION as disco_version
from syncord.api.ratelimit import RateLimiter
...
class Routes(object):
    ...
    CHANNELS_MESSAGES_REACTIONS_GET = (
        HTTPMethod.GET,
        CHANNELS + "/messages/{message}/reactions/{emoji}",
    )
    CHANNELS_MESSAGES_REACTIONS_CREATE = (
        HTTPMethod.PUT,
        CHANNELS + "/messages/{message}/reactions/{emoji}/@me",
    )
    CHANNELS_MESSAGES_REACTIONS_DELETE_ALL = (
        HTTPMethod.DELETE,
        CHANNELS + "/messages/{message}/reactions",
    )
    CHANNELS_MESSAGES_REACTIONS_DELETE_ME = (
        HTTPMethod.DELETE,
        CHANNELS + "/messages/{message}/reactions/{emoji}/@me",
    )
    CHANNELS_MESSAGES_REACTIONS_DELETE_USER = (
        HTTPMethod.DELETE,
        CHANNELS + "/messages/{message}/reactions/{emoji}/{user}",
    )
    CHANNELS_PERMISSIONS_MODIFY = (
        HTTPMethod.PUT,
        CHANNELS + "/permissions/{permission}",
    )
    CHANNELS_PERMISSIONS_DELETE = (
        HTTPMethod.DELETE,
        CHANNELS + "/permissions/{permission}",
    )
    ...
    GUILDS_MEMBERS_ROLES_ADD = (
        HTTPMethod.PUT,
        GUILDS + "/members/{member}/roles/{role}",
    )
    GUILDS_MEMBERS_ROLES_REMOVE = (
        HTTPMethod.DELETE,
        GUILDS + "/members/{member}/roles/{role}",
    )
    ...
    GUILDS_INTEGRATIONS_MODIFY = (
        HTTPMethod.PATCH,
        GUILDS + "/integrations/{integration}",
    )
    GUILDS_INTEGRATIONS_DELETE = (
        HTTPMethod.DELETE,
        GUILDS + "/integrations/{integration}",
    )
    GUILDS_INTEGRATIONS_SYNC = (
        HTTPMethod.POST,
        GUILDS + "/integrations/{integration}/sync",
    )
    ...
    def __init__(self, response, retries=None):
        ...
        try:
            ...
            if "code" in data:
                ...
                self.msg = "{} ({} - {})".format(
                    data["message"], self.code, self.errors
                )
            elif len(data) == 1:
...
class HTTPClient(LoggingClass):
    ...
    def call(self, route, args=None, **kwargs):
        ...
        filtered = {
            k: (v if k in ("guild", "channel") else "") for k, v in args.items()
        }
        ...
        if r.status_code < 400:
            ...
            self.log.warning(
                "Request failed with code %s: %s", r.status_code, r.content
            )
            ...
            self.log.warning(
                "Request to `{}` failed with code {}, retrying after {}s ({})".format(
                    url,
                    r.status_code,
                    backoff,
                    r.content,
