...
from syncord.client import Client, ClientConfig
...
class TestBot(TestCase):
    ...
    def test_command_abbreviation(self):
        ...
        self.assertDictEqual(
            self.bot.compute_group_abbrev(["test"]),
            {
                "test": "t",
            },
        )
    ...
    def test_many_commands(self):
        self.bot._commands = [
            Command(None, None, "test{}".format(i), "<test:str>") for i in range(1000)
        ]
    ...
    def test_group_and_command(self):
        ...
        msg.content = "?test a"
        commands = list(
            self.bot.get_commands_for_message(False, None, ["!", "?", ";"], msg)
        )
        self.assertEqual(commands[0][0], self.bot._commands[1])
        self.assertEqual(commands[1][0], self.bot._commands[0])
